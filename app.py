        ...
        with st.spinner("Fetching data..."):
            ...
            if not jc_df.empty:
                ...
                if bzid_col:
                    ...
                    # Try to find BZID in any column name
                    for col in jc_df.columns:
                        if 'bzid' in col.lower() or 'business' in col.lower():
                            jc_df["BZID"] = jc_df[col].astype(str).str.strip().str.upper()
                            break
            ...
            ] if not cash_df.empty else pd.DataFrame()
            ...
            ] if not jc_df.empty else pd.DataFrame()
            ...
            ] if not manual_df.empty else pd.DataFrame()
...
with tab2:
    ...
    try:
        bank_sheet_id = st.secrets["bank_transfer_sheet_id"]
    except KeyError:
...
with st.spinner(f"Searching for Ticket ID: {ticket_id}..."):
    ...
    bank_df = load_sheet(bank_sheet_id, "CD Refund Sheet")
